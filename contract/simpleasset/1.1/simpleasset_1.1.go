...
// 1. 외부모듈 포함
import (
	"bytes"         // 문자 버퍼를 사용하기 위한 라이브러리
	"encoding/json" // JSON구조를 사용 (marshal, unmarshal)
	"fmt"
	"strconv" // 문자열과 기본형 사이의 변환
	"time"    // 시간과 관련된
...
func (t *SimpleAsset) Set(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	if err != nil {
		return shim.Error("Failed to set asset: " + args[0] + " " + args[1])
	}
...
func (t *SimpleAsset) Get(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	if len(args) != 1 {
		return shim.Error("Incorrent arguments. Expecting a key")
	}

	value, err := stub.GetState(args[0])
	if err != nil {
		shim.Error("Failed to get asset: " + args[0] + " with error: " + err.Error())
	}
	if value == nil {
...
func (t *SimpleAsset) Transfer(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	// 1. 전달인자 확인
	if len(args) != 3 {
		return shim.Error("Incorrent arguments. Expecting a from_key, to_key and amount")
	}
	// 2. 보내는 이, 받는 이 GetState -> unmarshal
	...
	if err != nil {
		shim.Error("Failed to get asset: " + args[1] + " with error: " + err.Error())
	}
	if to_asset == nil {
